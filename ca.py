...
class CertificateHandler:
    ...
    def create_certificate(subject, issuer, public_key, private_key, is_ca=False):
        subjected = x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, subject)]) if isinstance(subject, str) else subject
        issued = x509.Name([x509.NameAttribute(NameOID.COMMON_NAME, issuer)]) if isinstance(issuer, str) else issuer
...
class SpecialCertificateHandler(CertificateHandler):
    ...
    def create_intermediate(self, root_key, root_cert, name, key_type="RSA"):
        ...
        cert = self.create_certificate(root_cert.subject, name, key.public_key(), root_key, is_ca=True)
        return key, cert

    def create_cross_sign(self, to_be_signed_key, to_be_signed_cert, signer_key, signer_name):
        cert = self.create_certificate(to_be_signed_cert.subject, signer_name, to_be_signed_key.public_key(), signer_key)
        return cert
